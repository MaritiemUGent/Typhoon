...
text(0,.2,'STATE: ');
text(0,.15,'\theta [deg]: ');   text(.15,.15,num2str(state.theta*180/pi));
text(0,.1,'\psi [deg]: ');     text(.15,.1,num2str(state.psi*180/pi));
text(0,.05,'phi: ');			text(.15,.05,num2str(state.phi*180/pi));
text(0,.0,'STW: ');       text(.15,.05,num2str(state.STW));

try
 text(0,-.05,'Elavation: ');       text(.15,.0,num2str(state.ELA));
end
 text(0,-.1,'Density: ');      text(.15,-.05,num2str(state.rho));
...
%[void sos voild]=ISAtmosphere(state.ELA);
%Mach=state.STW/sos;
...
text(0.4,.90,'\theta [deg]: '); 	text(.55,.9,num2str(state.theta*180/pi));
text(0.4,.85, '\psi [deg]: ');		text(.55,.85,num2str(state.psi*180/pi));
text(0.4,.8, '\phi [deg]: ');		text(.55,.8,num2str(state.phi*180/pi));
text(0.4,.75,'STW: ');				text(.55,.75,num2str(state.STW));
...
text(0,.65,'CL_{\theta}');		    text(0.15,.65,num2str(results.CL_a));
text(0,.6,'CL_{\psi}');		    text(0.15,.6,num2str(results.CL_b));
...
text(0,.3,'Cl_{\theta}');		    text(0.15,.3,num2str(results.Cl_a));
text(0,.25,'Cl_{\psi}');		    text(0.15,.25,num2str(results.Cl_b));
...
text(0.35,.65,'CD_{\theta}');	text(0.5,.65,num2str(results.CD_a));
text(0.35,.6,'CD_{\psi}');	text(0.5,.6,num2str(results.CD_b));
...
text(0.35,.3,'Cm_{\theta}');	text(.5,.3,num2str(results.Cm_a));
text(0.35,.25,'Cm_{\psi}');	text(0.5,.25,num2str(results.Cm_b));
...
text(0.7,.65,'CY_{\theta}');	text(0.85,.65,num2str(results.CY_a));
text(0.7,.6,'CY_{\psi}');		text(0.85,.6,num2str(results.CY_b));
...
text(0.7,.3,'Cn_{\theta}');	text(.85,.3,num2str(results.Cn_a));
text(0.7,.25,'Cn_{\psi}');	text(0.85,.25,num2str(results.Cn_b));
...
text(0.4,.90,'\theta: '); 	text(.55,.9,num2str(state.theta*180/pi));
text(0.4,.85,'\psi: ');     text(.55,.85,num2str(state.betha*180/pi));
text(0.4,.8,'Airspeed: ');  text(.55,.8,num2str(state.STW));
...
%}
  ...
  case 2
	...
	case 3
        disp('""""""""""""""""""""""""""""""""""""""""""""""""""""""');
        disp('Available theta sweep solution result files are: ')
        cd(settings.odir)
            dir *-Cx_theta.mat
        cd(settings.hdir)
        data=questions(10);
   ...
   figure(15) 
   subplot(3,2,3), plot(results.theta_sweep,squeeze(results.matrix(2,1,:)));
   xlabel('Alpha [rad]')
   ylabel('CD')
   
   subplot(3,2,5), plot(results.theta_sweep,squeeze(results.matrix(3,1,:)));
   xlabel('Alpha [rad]')
   ylabel('CY')
   
   subplot(3,2,2), plot(results.theta_sweep,squeeze(results.matrix(4,1,:)));
   xlabel('Alpha [rad]')
   ylabel('Cl')
   
   subplot(3,2,4), plot(results.theta_sweep,squeeze(results.matrix(5,1,:)));
   xlabel('Alpha [rad]')
   ylabel('Cm')
   
   subplot(3,2,6), plot(results.theta_sweep,squeeze(results.matrix(6,1,:)));
   xlabel('Alpha [rad]')
   ylabel('Cn')
   
   subplot(3,2,1), plot(results.theta_sweep,squeeze(results.matrix(1,1,:)));
   xlabel('Alpha [rad]')
   ylabel('CL')
  
  axes
  text(0.0,1.05,'Coefficient dependency on theta');
  axis off  
  ...
  figure(17)
    ...
    try 
        ...
        figure(18)
        plot(theta1,delta1)
        xlabel('Angle of attack, \theta, [deg].')
...
case 4
        disp('""""""""""""""""""""""""""""""""""""""""""""""""""""""');
        disp('Available psi sweep solution result files are: ')
        cd(settings.odir)
            dir *-Cx_psi.mat
        cd(settings.hdir)
        data=questions(10);
  ...
  axes
  text(0.0,1.05,'Coefficient dependency on psi (sideslip)')
  axis off
...
case 10
 ...
 case 11 %***
    ...
    for j=1:1:numbmhs %***
        ...
        title(['Mach ',num2str(MSTWTDRG(j+1,1)/10)]) %***
        xlabel('C_L') %***
        ylabel('Drag (Counts)') %***
 ...
 case 12 %***
    ...
    case 15
        ...
        text(0,.7,'CZ_{\theta_{dot}}: ');  		text(.2,.7,num2str(results.CZ_a_dot));
        text(0,.65,'Cm_{\theta_{dot}}: ');  		text(.2,.65,num2str(results.Cm_a_dot));
    
        text(0,.6,'CY_{\psi_{dot}}: ');  		text(.2,.6,num2str(results.CY_b_dot));
        text(0,.55,'Cn_{\psi_{dot}}: ');  		text(.2,.55,num2str(results.Cn_b_dot));
...
text(0,.2,'STATE: ');
text(0,.15,'\theta [deg]: ');   text(.15,.15,num2str(state.theta*180/pi));
text(0,.1, '\psi [deg]: ');    text(.15,.1,num2str(state.betha*180/pi));
text(0,.05,'Airspeed: ');       text(.15,.05,num2str(state.STW));
try
...
text(0.3,.0,'\theta_{dot} [deg/s]: ');  text(.5,.0,num2str(state.adot*180/pi));
text(0.3,-.05,'\psi_{dot}  [deg/s]: '); text(.5,-.05,num2str(state.bdot*180/pi));
...
text(0.4,.90,'\theta [deg]: '); 	text(.55,.9,num2str(state.theta*180/pi));
text(0.4,.85, '\psi [deg]: ');     text(.55,.85,num2str(state.betha*180/pi));
text(0.4,.80,'Airspeed: ');         text(.55,.8,num2str(state.STW));
...
text(0.65,.75,'\theta_{dot} [deg/s]: ');  text(.85,.75,num2str(state.adot*180/pi));
text(0.65,.70,'\psi_{dot}  [deg/s]: ');  text(.85,.70,num2str(state.bdot*180/pi));

text(0,.6,'CL derivatives : ');			

text(0,.55,'CL_{\theta}');		    text(0.15,.55,num2str(results.CL_a));
text(0,.5,'CL_{\psi}');		    text(0.15,.5,num2str(results.CL_b));
...
text(0,.2,'Cl_{\theta}');		    text(0.15,.2,num2str(results.Cl_a));
text(0,.15,'Cl_{\psi}');		    text(0.15,.15,num2str(results.Cl_b));
...
text(0.35,.55,'CD_{\theta}');	text(0.5,.55,num2str(results.CD_a));
text(0.35,.5,'CD_{\psi}');	text(0.5,.5,num2str(results.CD_b));
...
text(0.35,.2,'Cm_{\theta}');	text(.5,.2,num2str(results.Cm_a));
text(0.35,.15,'Cm_{\psi}');	text(0.5,.15,num2str(results.Cm_b));
...
text(0.7,.55,'CY_{\theta}');	text(0.85,.55,num2str(results.CY_a));
text(0.7,.5,'CY_{\psi}');		text(0.85,.5,num2str(results.CY_b));
...
text(0.7,.2,'Cn_{\theta}');     text(.85,.2,num2str(results.Cn_a));
text(0.7,.15,'Cn_{\psi}');     text(0.85,.15,num2str(results.Cn_b));
...
text(0.65,.75,'\theta_{dot}: ');  text(.85,.75,num2str(state.adot*180/pi));
text(0.65,.70,'\psi_{dot}: ');  text(.85,.70,num2str(state.bdot*180/pi));
